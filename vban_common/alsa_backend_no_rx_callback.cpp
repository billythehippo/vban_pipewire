...
void* rxThread(void* param)
{
    ...
    while (1)
    {
        while (poll(&stream->polldesc, 1, 500))
        {
            ...
            if (packetlen)
            {
                iprx = stream->vban_si.sin_addr.s_addr; // get senrder's ip
                if (stream->packet.header.vban==VBAN_HEADER_FOURC) // check for VBAN packet
                {
                    ...
                    case VBAN_PROTOCOL_AUDIO:
                        if (stream->ip == 0) // still not started
                        {
                            ...
                            if ((strncmp(stream->packet.header.streamname, stream->header.streamname, VBAN_STREAM_NAME_SIZE)==0)&&(stream->packet.header.format_nbc==stream->header.format_nbc)) stream->ip = iprx;
                        }
                        if ((strncmp(stream->packet.header.streamname, stream->header.streamname, VBAN_STREAM_NAME_SIZE)==0)&&(stream->ip == iprx)&&
                                ...
                                (stream->packet.header.nuFrame    != stream->header.nuFrame))
                        {
                            ...
                            if (samplesize==0) // INIT
                            {
                                nsamples = stream->packet.header.format_nbs + 1;
                                samplesize = (stream->header.format_nbc + 1)*(VBanBitResolutionSize[stream->header.format_bit&VBAN_BIT_RESOLUTION_MASK]);
                            ...
                            {
                                wroteframes = snd_pcm_writei(stream->handle->handle, stream->packet.data, stream->packet.header.format_nbs + 1);
                                if (wroteframes<0) wroteframes = snd_pcm_recover(stream->handle->handle, wroteframes, 0);
                                if (wroteframes<0) fprintf(stderr, "Error: writei failed!\n");
                                if (wroteframes<=stream->packet.header.format_nbs) fprintf(stderr, "Warning: Short write\n");//*/
                                /*srcptr = &stream->packet.data[0];//&((char*)&stream->packet)[VBAN_HEADER_SIZE];
                                for (sample=0; sample<nsamples; sample++)
                                {
                                    if(jack_ringbuffer_write_space(stream->ringbuffer)>=samplesize) jack_ringbuffer_write(stream->ringbuffer, srcptr, samplesize);
                                    srcptr+= samplesize;
                                }//*/
                                /*cnt = 0;
                                while(cnt<nsamples)
                            ...
                            else
                            {
                                ...
                                wroteframes = snd_pcm_writei(stream->handle->handle, stream->rxbuf, nsamples);
                                if (wroteframes<0) wroteframes = snd_pcm_recover(stream->handle->handle, wroteframes, 0);
                                if (wroteframes<0) fprintf(stderr, "Error: writei failed!\n");
                                if (wroteframes<=stream->packet.header.format_nbs) fprintf(stderr, "Warning: Short write\n");//*/
                            }
...
void pbCallback(snd_async_handler_t *pcm_callback)  // playback callback
{
    ...
    char* ptr;
    int ptr_step;
    char ssize = VBanBitResolutionSize[pcm_handle->format_vban];
    int buflen = pcm_handle->rxbuflen;
    int wroteframes;
    jack_ringbuffer_t* ringbuf = pcm_handle->ringbuffer;
    snd_pcm_sframes_t avail;

    ptr_step = nframes*nbchannels*ssize;
    ptr = pcm_handle->rxbuf;

    for(ptr = pcm_handle->rxbuf; ptr<(pcm_handle->rxbuf+buflen); ptr+=ptr_step)
    {
        if (jack_ringbuffer_read_space(ringbuf)>=ptr_step) jack_ringbuffer_read(ringbuf, ptr, ptr_step);
        ptr+= ptr_step;
    }
    ...
    if (avail>=nframes)
    {
        wroteframes = snd_pcm_writei(handle, (void*)&pcm_handle->rxbuf, nframes); //*nbchannels*ssize
...
int pipe2alsa(int argc, char *argv[])
{
    ...
    stream.rxbuflen = playbackHandle.nframes*playbackHandle.nbchannels*VBanBitResolutionSize[stream.format_vban];
    stream.rxbuf = (char*)malloc(stream.rxbuflen);
    memset(stream.rxbuf, 0, stream.rxbuflen);
    for (index=0; index<stream.redundancy; index++)
    {
        if (snd_pcm_writei(playbackHandle.handle, stream.rxbuf, stream.rxbuflen) < 0) //playbackHandle.nframes
        {
    ...
    stream.handle = &playbackHandle; // TEMPORARY!!!
